...
function ids = read_list_from_file( gt_set )
    ...
    index_file = fullfile(db_root_dir,[gt_set '.txt']);
    if ~exist(index_file,'file')
        error(['File ''' index_file ''' not found... did you set correctly the path in db_root_dir to the folder where DAVIS is found?'])
    end
